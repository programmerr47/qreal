	...
	if (NULL == node->parentItem() || !selected.contains(node->parentItem())) {
		...
		foreach (QGraphicsItem *item, items(newParentInnerPoint)) {
			...
			if (e != NULL && e != node && !selected.contains(item)) {
				// Ð¿Ñ€Ð¾Ð²ÐµÑ€ÐºÐ°, Ð¼Ð¾Ð¶Ð½Ð¾ Ð»Ð¸ Ð´Ð¾Ð±Ð°Ð²Ð»ÑÑ‚ÑŒ Ð½Ð°Ñˆ ÑÐ»ÐµÐ¼ÐµÐ½Ñ‚ Ð² Ð½Ð°Ð¹Ð´ÐµÐ½Ð½Ð¾Ð³Ð¾ Ñ€Ð¾Ð´Ð¸Ñ‚ÐµÐ»Ñ
...
//íà èñïðàâëåíèå - èñïîëüçóåòñÿ EditorManager â MainWindow
